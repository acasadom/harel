"""SurrealStore unit tests, backed by SurrealDB's in-process `mem://` engine
(bundled with the `surrealdb` package — no server, no Docker).

Same ExecutionStore contract as the SQL backends — version/CAS, transactional
outbox, dedupe, spawns, timers — and structurally the closest to PostgresStore:
real ACID transactions, so a commit is one server-side BEGIN…COMMIT block with a
CAS gated by THROW. The last two tests drive the full DistributedRunner + Worker
pipeline over a **pure-Surreal** backend (SurrealStore + SurrealTransport).

NOTE: each `mem://` connection is an isolated instance (data is not shared across
connections), so the store and transport in the pipeline tests share ONE client,
and there is no multi-client concurrency test here — that is covered against a
real SurrealDB server in test/integration/test_surreal_store.py.
"""
...
from harel.engine.store import StoreConflict, SurrealStore, TimerOp  # noqa: E402
...
def test_commit_outbox_on_first_insert(store):
    ...
    # (version 0 -> the CREATE branch of the CAS block), so it must be carried
    e = Execution(definition_id="d")
...
def test_stale_write_does_not_leak_outbox(store):
    # the CAS THROW must roll back the whole transaction, including the outbox
    e = Execution(definition_id="d")
    store.save(e)  # version -> 1
    other = store.load(e.id)
    store.save(other)  # version -> 2 (e is now stale at 1)
...
def test_timers_schedule_due_and_cancel(store):
    e = Execution(definition_id="d")
    # paths carry dots (the node separator) — bound as params, no encoding needed
    ...
    # re-schedule the same path replaces the fire_at (composite record id)
...
# --- the full pipeline over a PURE-SURREAL backend (SurrealStore + SurrealTransport) ---------


def test_pipeline_flat_pure_surreal(client):
    defn = definition_from_dsl(FLAT, "M")
    store = SurrealStore(client)
...
def test_pipeline_orthogonal_pure_surreal(client):
