...
class AsyncSurrealStore:
    ...
    async def load(self, execution_id: str) -> Optional[Execution]:
        res = await self._db.query(
            "SELECT data FROM type::thing('executions',$id)", {"id": execution_id}
        )
    ...
    async def commit(
        ...
        if processed_event_id is not None:
            stmts.append(
                "UPSERT type::thing('processed',[$id,$pe]) SET execution_id=$id, event_id=$pe"
            )
        ...
        try:
            ...
            exe.version = old
            res = await self._db.query(
                "SELECT version FROM type::thing('executions',$id)", {"id": exe.id}
            )
    ...
    async def pending_outbox(self) -> list[OutboxEntry]:
        rows = await self._db.query(
            "SELECT seq, target_id, event FROM outbox ORDER BY seq ASC"
        )
        return [
            OutboxEntry(r["seq"], r["target_id"], Event.model_validate_json(r["event"]))
            for r in rows
        ]
